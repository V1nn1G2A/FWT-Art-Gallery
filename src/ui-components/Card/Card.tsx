...
// Рефакторинг: изменил названия компонента
function Card({ theme = 'light', onClick, ...picture }: CardProps) {
  const [active, setActive] = useState(false);
  const baseImageUrl = 'https://internship-front.framework.team/';

  return (
    <li className={`${styles.card} ${styles[`card--${theme}`]}`}>
      <img
      ...
      <div className={styles.card_info}>
        <h3 className={styles.card_title}>{picture.name.toUpperCase()}</h3>
